...
class TestCheckpointIntegrity(unittest.TestCase):
    ...
    def test_checkpoint_save_creates_file(self):
        ...
        # Checkpoint is saved after evaluation; first checkpoint is for generation 0.
    ...
    def test_checkpoint_generation_preserved(self):
        ...
        Checkpoint N contains the evaluated population for generation N.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        ...
        # After run(fn, 5), generations 0-4 are evaluated.
        # Last checkpoint is for generation 4.
    ...
    def test_checkpoint_preserves_population_size(self):
        ...
        # Checkpoint saves evaluated population; last checkpoint is gen 1.
        # pop.population is the post-reproduction gen-2 population, so compare
        # against the configured pop_size instead.
        checkpoint_file = f'{self.checkpoint_prefix}1'
        restored_pop = neat.Checkpointer.restore_checkpoint(checkpoint_file)

        self.assertEqual(len(restored_pop.population), self.config.pop_size,
    ...
    def test_checkpoint_preserves_genome_keys(self):
        ...
        so we snapshot the keys via a reporter at evaluation time and compare.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        snapshot = CheckpointSnapshotReporter(1)
        pop.add_reporter(snapshot)
        ...
        # The snapshot captured the population at post_evaluate for gen 1,
        # which is exactly what the checkpoint saved.
        self.assertEqual(restored_keys, set(snapshot.snapshot.keys()),
    ...
    def test_checkpoint_preserves_genome_structure(self):
        ...
        Restores from the last checkpoint and verifies that every genome
        in the restored population matches the checkpoint's state.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        ...
        # Last evaluated generation is pop.generation - 1.
        last_eval_gen = pop.generation - 1
        checkpoint_file = f'{self.checkpoint_prefix}{last_eval_gen}'
        restored_pop = neat.Checkpointer.restore_checkpoint(checkpoint_file)

        # Restore a second copy to get the ground-truth structures.
        restored_pop2 = neat.Checkpointer.restore_checkpoint(checkpoint_file)

        for gid, genome in restored_pop2.population.items():
            self.assertIn(gid, restored_pop.population)
            rg = restored_pop.population[gid]
            self.assertEqual(len(rg.nodes), len(genome.nodes),
                           f"Genome {gid} should have same number of nodes")
            self.assertEqual(len(rg.connections), len(genome.connections),
    ...
    def test_checkpoint_preserves_fitness_values(self):
        ...
        Checkpoints save after evaluation, so every genome has a fitness value.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        ...
        # Every genome in the checkpoint should have a non-None fitness.
        for gid, genome in restored_pop.population.items():
            self.assertIsNotNone(genome.fitness,
                               f"Genome {gid} fitness should be preserved (not None)")
    ...
    def test_checkpoint_preserves_species_count(self):
        ...
        Restoring from the same checkpoint twice should yield the same species count.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        ...
        self.assertGreater(len(restored_pop.species.species), 0,
    ...
    def test_checkpoint_preserves_species_membership(self):
        ...
        Every genome in the restored population should belong to a species.
        """
        pop = neat.Population(self.config)
        checkpointer = neat.Checkpointer(1, filename_prefix=self.checkpoint_prefix)
        pop.add_reporter(checkpointer)
        ...
        # Build genome→species mapping from restored population
        membership = {}
        for sid, species in restored_pop.species.species.items():
            for gid in species.members:
                membership[gid] = sid

        # Every genome in the population should be assigned to a species
        for gid in restored_pop.population:
            self.assertIn(gid, membership,
                         f"Genome {gid} should be in a species")
    ...
    def test_checkpoint_genome_indexer_continues(self):
        ...
        max_genome_id = max(restored_pop.population.keys())
    ...
    def test_checkpoint_preserves_random_state(self):
        ...
        # The random state should be from generation 0, not from "before_restore"
    ...
    def test_checkpoint_restore_with_same_config(self):
        ...
        self.assertGreater(restored_pop.generation, 1,
    ...
    def test_checkpoint_multiple_generations(self):
        ...
        With interval=2, the first checkpoint fires after 2 evaluations
        (generations 0 and 1), so the first checkpoint is for generation 1,
        then generation 3, etc.
        """
        pop = neat.Population(self.config)
        ...
        # With interval=2: checkpoints at generations 1 and 3.
        self.assertFalse(os.path.exists(f'{self.checkpoint_prefix}0'),
                        "Checkpoint at generation 0 should not exist")
    ...
    def test_checkpoint_can_restore_any_generation(self):
        ...
        # With interval=1, checkpoints exist for generations 0, 1, 2, 3.
        for gen in [0, 1, 2, 3]:
    ...
    def test_checkpoint_after_first_generation(self):
        ...
        # run(1) evaluates generation 0; checkpoint is saved after evaluation.
        checkpoint_file = f'{self.checkpoint_prefix}0'
        self.assertTrue(os.path.exists(checkpoint_file),
    ...
    def test_checkpoint_file_format_is_gzipped_pickle(self):
        ...
        # Should be a tuple with 6 elements (generation, config, population,
        # species_set, random_state, best_genome)
        self.assertIsInstance(data, tuple, "Checkpoint data should be a tuple")
        self.assertEqual(len(data), 6, "Checkpoint should have 6 elements")

        generation, config, population, species_set, rndstate, best_genome = data
