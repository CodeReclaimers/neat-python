...
def create_ffphenotype(chromo):
    ...
    for id in  chromo.node_order:    
        ...
        network.setNeuronParameters(value, chromo.node_genes[id - 1].bias, chromo.node_genes[id - 1].response, 0)
        value += 1
    # output nodes
    for ng in chromo.node_genes[num_inputs:num_outputs+num_inputs]:
        mapping[ng.id] = value
        #print 'Output params: ', value, ng.bias, ng.response, 1
        network.setNeuronParameters(value, ng.bias, ng.response, 1)
        value += 1
    ...
    for cg in chromo.conn_genes:
        if cg.enabled:
            if cg.innodeid-1 < num_inputs:
                ...
                network.SetConnectionWeight(mapping[cg.innodeid], mapping[cg.outnodeid], cg.weight)
...
if __name__ == "__main__":
    ...
    network.setNeuronParameters(0, 0.0, 4.924273, 0)
    network.setNeuronParameters(1, 0.0, 4.924273, 1)

    network.set_sensory_weight(1, 1, -0.09569)
    network.set_sensory_weight(0, 0, 1.0)
    network.SetConnectionWeight(0,1,0.97627)
