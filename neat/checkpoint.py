...
class Checkpointer(BaseReporter):
    ...
    def end_generation(self, config, population, species_set, best_genome):
        ...
        if checkpoint_due:
            self.save_checkpoint(config, population, species_set, self.current_generation, best_genome)
    ...
    def save_checkpoint(self, config, population, species_set, generation, best_genome):
        ...
        with gzip.open(filename, 'w', compresslevel=5) as f:
            data = (generation, config, population, species_set, random.getstate(), best_genome)
    ...
    def restore_checkpoint(filename):
        ...
        with gzip.open(filename) as f:
            generation, config, population, species_set, rndstate, best_genome = pickle.load(f)
