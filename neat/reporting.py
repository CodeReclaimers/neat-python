...
class StdOutReporter(BaseReporter):
    ...
    def end_generation(self, config: Config, population: Dict[int, DefaultGenome], species_set: DefaultSpeciesSet) -> None:
        ng: int = len(population)
        ns: int = len(species_set.species)
        if self.show_species_detail:
            print('Population of {0:d} members in {1:d} species:'.format(ng, ns))
            print("   ID   age  size  fitness  adj fit  stag")
            print("  ====  ===  ====  =======  =======  ====")
            for sid in sorted(species_set.species):
                s: Species = species_set.species[sid]
                a: int = self.generation - s.created
                n: int = len(s.members)
                f: str = "--" if s.fitness is None else "{:.1f}".format(s.fitness)
                af: str = "--" if s.adjusted_fitness is None else "{:.3f}".format(s.adjusted_fitness)
                st: int = self.generation - s.last_improved
                print(
                    "  {: >4}  {: >3}  {: >4}  {: >7}  {: >7}  {: >4}".format(sid, a, n, f, af, st))
        else:
            print('Population of {0:d} members in {1:d} species'.format(ng, ns))

        elapsed: float = time.time() - self.generation_start_time
        self.generation_times.append(elapsed)
        self.generation_times = self.generation_times[-10:]
        average: float = sum(self.generation_times) / len(self.generation_times)
